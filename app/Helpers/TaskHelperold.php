...
class TaskHelper
{
    ...
    public static function userLeaveALLReport1($userId, $leaveCatId, $startdate, $enddate)
    {
        ...
        $userLeaves_full_leaves = LeaveApplication::select(DB::raw('sum(leave_days) as total'))->where('user_id', $userId)->where('leave_category_id', $leaveCatId)->where('application_status', '2')->whereDate('leave_start_date', '>=', $startdate)->whereDate('leave_end_date', '<=', $enddate)->where('part_of_day',1)->first();

        $userLeaves_half_leaves = LeaveApplication::select(DB::raw('sum(leave_days) as total'))->where('user_id', $userId)->where('leave_category_id', $leaveCatId)->where('application_status', '2')->whereDate('leave_start_date', '>=', $startdate)->whereDate('leave_end_date', '<=', $enddate)->where('part_of_day','>',1)->first();

        $userLeaves =   $userLeaves_full_leaves->total +  ($userLeaves_half_leaves->total/2);
...
//           }
    ...
    //anamol
    public static function getUserAttendanceNew($user_id,$date_in){
        ...
        $attendance_records=ShiftAttendance::where(function ($q) use ($user_id){
            if($user_id){
...
$attendance=$attendance_records->groupby('date');
        return $attendance;
    }
