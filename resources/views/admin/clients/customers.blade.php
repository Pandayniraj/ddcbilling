...
@section('content')
    <section class="content-header" style="margin-top: -35px; margin-bottom: 20px">
        ...
        <i class="fa fa-info"></i> Create {{$page_title ?? "Customers"}} Account.


        {!! MenuBuilder::renderBreadcrumbTrail(null, 'root', false)  !!}
    </section>
    @php $relation_type=\Request::get('relation_type'); @endphp
    <div class='row'>
        <div class='col-md-12'>
            <!-- Box -->
            ...
            <div class="box box-primary">
                <div class="box-header with-border">
                    ...
                    <a class="btn btn-default btn-sm"
                       href="{!! route('admin.clients.create') !!}?relation_type={{$relation_type}}"
                    ...
                    <a class="btn btn-default btn-sm" href="{!! route('admin.import-export-clients.index') !!}"
                    ...
                    <div class="col-md-4 col-sm-4 col-lg-4" style="float: right;margin-top: 4px">
                        <div class="input-group">
                            ...
                            <div class="input-group-btn">
                                ...
                                <button type="button" class="btn btn-primary btn-flat btn-sm" id="search"><i
                                        class="fa fa-search"></i>&nbsp;Filter
                                </button>
                            </div>
                            <div class="input-group-btn">
                                <button type="button" class="btn btn-default btn-flat btn-sm" id="clear"><i
                                        class="fa fa-close (alias)"></i>&nbsp; Clear
                                </button>
                            </div>
                        </div>
                    </div>
                        ...
                        <div class="table-responsive">
                            <table class="table table-hover table-bordered" id="clients-table">
                                ...
                                <tr>
                                    ...
                                    <th>{{ $page_title ?? "Customers" }} Name</th>
                                ...
                                @foreach($clients as $client)
                                    <tr>
                                        ...
                                        <td class=""><span style="font-size: 17px">
                                            ...
                                            @if($client->image)
                                                    <img src="{{asset($client->image)}}" class="p-image" id="blah"
                                                         src="#" alt="your image"/>
                                                @else
                                        ...
                                        {{-- <td>{{$client->deposit_amount}}</td> --}}
                                        @php
                                            $amount=\App\Models\CustomerDeposit::where('client_id',$client->id)->orderby('id','desc')->first()->closing??0;
                                        @endphp
                                        <td>{{$amount}}</td>
                                        ...
                                        <td>
                                            @if ( $client->isEditable() || $client->canChangePermissions() )
                                                <a href="{!! route('admin.clients.edit', $client->id) !!}?relation_type={{$client->relation_type}}"
                                                   title="{{ trans('general.button.edit')}}"><i class="fa fa-edit"></i></a>
                                            @else
                                            ...
                                            @if ( $client->enabled )
                                                <a href="{!! route('admin.clients.disable', $client->id) !!}?relation_type={{$client->relation_type}}"
                                                   title="{{ trans('general.button.disable') }}"><i
                                                        class="fa fa-check-circle enabled"></i></a>
                                            @else
                                                <a href="{!! route('admin.clients.enable', $client->id) !!}?relation_type={{$client->relation_type}}"
                                                   title="{{ trans('general.button.enable') }}"><i
                                                        class="fa fa-ban disabled"></i></a>
                                            @endif

                                            @if ( $client->isDeletable() )
                                                <a href="{!! route('admin.clients.confirm-delete', $client->id) !!}?relation_type={{$client->relation_type}}"
        ...
        @section('body_bottom')
            ...
            <script language="JavaScript">
                ...
                $('#clear').click(function () {
                    window.location.href = "{!! url('/') !!}/admin/customer?relation_type={{ \Request::get('relation_type') }}";
                })
                $('.searchable').select2();
            </script>
