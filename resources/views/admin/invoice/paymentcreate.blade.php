                                    ...
                                    <div class="form-group">
                                        ...
                                        <div class="input-group ">
                                            <input type="text" name="amount" placeholder="Amount" id="price_value"
                                                value="{{ $payment_remain }}" class="form-control" required="required">
                                            <div class="input-group-addon">
                                ...
                                <div class="form-group">
                                    ...
                                    <div class="col-md-8">
                                        <select class='form-control searchable select2' name='payment_method'>
                                            <?php
                                            //Sunny_deptors
                                            $groups = \App\Models\COALedgers::orderBy('code', 'asc')
                                                ->where('group_id', '13')
                                                ->where('org_id', \Auth::user()->org_id)
                                                ->get();
                                            foreach ($groups as $grp) {
                                                echo '<option value="' . $grp->id . '"' . (isset($client) && $grp->name == $client->type ? 'selected="selected"' : '') . '>' . $grp->name . '</option>';
                                            }
                                            ?>
                                        </select>
                                    </div>
