...
class InvoiceController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        $bill_no = \DB::select("SELECT MAX(Convert(`bill_no`,SIGNED)) as last_bill from invoice WHERE fiscal_year_id = '$ckfiscalyear->id' AND  org_id = '$org_id'  limit 1");
...
// dd($invoice);
    ...
    public function productwisereport(Request $request){
        ...
        if($request->startdate && $request->startdate !=""){
            ...
            $products=\App\Models\Product::where('org_id',\Auth::user()->id)->pluck('name','id');
            ...
            if($op=="excel"){
                ...
                $pdf = \PDF::loadView('admin.reports.productwisereportPDF',compact('stock','data', 'products', 'startdate','enddate','organization'))->setPaper('a3', 'landscape');
        ...
        return view('admin.reports.dailysalesreport',compact('page_title','page_description'));
    }
    ...
    public function transactionreports(Request $request){
        ...
        if($request->startdate && $request->startdate !=""){
            ...
            $clients=\App\Models\Client::where('org_id',\Auth::user()->id)->select('name','id','relation_type')->get();
// dd($clients);
...
// dd("check");
                ...
                $pdf = \PDF::loadView('admin.reports.customerwisereportPDF',compact( 'detail_transaction','clients', 'startdate','enddate','organization'))->setPaper('a4', 'landscape');
                $file = $startdate . '_' . $enddate . '_' . str_replace(' ', '_', $organization->organization_name) . '.pdf';
                if (\File::exists('reports/' . $file)) {
                    \File::Delete('reports/' . $file);
                }
        ...
        return view('admin.reports.transactionreports',compact('page_title','page_description'));
    }
